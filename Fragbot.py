...
def handleMsg(this,message,messagePosition,*_):
  ...
  if "invited" in message:
    ...
    sleep(0.5)
    mcbot.chat('/pc How many seconds do you need for this bot.(This Bot will leave after 5 seconds')
    sleep(1)
    mcbot.chat('/pc Select One(type the letter)        a)15      b)30    c)45     d)60')
    ye = re.search(r'(?<=:)[^.]*',message)
    mcbot.chat("/p leave")
...
async def cm(ctx):
  ...
  if cash==2:
    ...
    embedVar = discord.Embed(name="You Won ",value=" ",color=0x00ff00)
    embedVar.add_field(name="Yon won somtething" , value="**Your number:**"f'{cash}', inline=False)
    ...
    embedVar.add_field(name="NICE!")
    await ctx.send(f'{rle.mention,ctx.message.author.mention}', embed=embedVar)

bot.run('OTAxNjM0NTMzMTM1MzUxODI5.YXSumA.JzRY7pq073duj-Bg_BCaxQ9PRMU')
